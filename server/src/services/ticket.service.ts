...
import { canRoleTransition } from "../domain/permissions";
...
function serialize(ticket: TicketRecord) {
  ...
  return {
    ...
    allowedTransitions: Object.prototype.hasOwnProperty.call(
      transitions,
      ticket.status,
    )
      ? transitions[ticket.status as TicketStatus]
      : [],
  };
}
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const tickets = await tx.repairTicket.findMany({
        ...
        take: query.limit,
        orderBy: [{ [query.sortBy]: query.order }, { id: query.order }],
        include,
      });
      const groups = await tx.repairTicket.groupBy({
        by: ["status"],
        _count: { _all: true },
      });
...
export async function update(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (ticket.version !== input.version)
      throw new HttpError(
        409,
        "รายการถูกแก้ไขแล้ว กรุณาโหลดข้อมูลล่าสุดก่อนบันทึก",
      );
    if (!canTransition(ticket.status, input.status))
      throw new HttpError(409, "ไม่สามารถเปลี่ยนสถานะตามลำดับนี้ได้");
    if (
      !canRoleTransition(user.role, ticket.status as TicketStatus, input.status)
    )
      throw new HttpError(403, "Role นี้ไม่มีสิทธิ์เปลี่ยนเป็นสถานะที่เลือก");
